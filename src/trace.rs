...
///   * The [`trace`] implementation can trace (maximum once) every [`Cc`] instance *exclusively* contained by `self`.
...
///     [`Cc`] instances the same amount of times.  
...
///   * To be compatible with the derive macro, the implementing type should not be a reference or raw pointer. Thus, for instance, given a type `A`,
