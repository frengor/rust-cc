...
fn test_auto_collect() {
...

    {
        ...
        let executions_count = execution_count().unwrap();
        drop(_traceable);
        assert_eq!(executions_count, execution_count().unwrap(), "Collected but shouldn't have collected.");

        let _ = Cc::new(Traceable {
            inner: None,
            _big: Default::default(),
        }); // Collection should be triggered by allocations
        assert_ne!(executions_count, execution_count().unwrap(), "Didn't collected");
    }
